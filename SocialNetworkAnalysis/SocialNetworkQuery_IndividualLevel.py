...
for id in df['ID'].unique():
    ...
    edges = filter_df[['ID','Related ID']].values.tolist()
    ...
    rel_df = pd.merge(rel_df, filter_df, how='left', on='ID')
    rel_df = rel_df[['ID','Related ID','X','Y','Degree Centrality','Betweenness Centrality','Closeness Centrality','Eigenvector Centrality']]
    ...
    df2.append(rel_df)
    edges2.append(edge_weights)

    # cursor = conn.cursor()
    # cursor.executemany("""INSERT INTO [dbo].[SocialNetworkCoordinates] ([ID],[Related ID],[X],[Y],[Degree Centrality],[Betweenness Centrality],[Closeness Centrality],[Eigenvector Centrality]) VALUES (?,?,?,?,?,?,?,?)""",rel_df.values.tolist())
    # cursor.commit()
...
df2.to_excel(writer,sheet_name='Nodes')
edges2 = pd.DataFrame(df2)
edges2.to_excel(writer,sheet_name='Edges')
